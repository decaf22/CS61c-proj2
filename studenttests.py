...
class TestAbsLoss(unittest.TestCase):
    ...
    def test_exception_length_zero(self):
        # load the test for abs_loss.s
        t = AssemblyTest(self, "../coverage-src/abs_loss.s")
        t.input_scalar("a2", -1)
        t.call("abs_loss")
...
class TestInitializeZero(unittest.TestCase):
    def test_simple(self):
        ...
        t.input_scalar("a0", 1) # we want the input to be 1
    ...
    def test_exception_malloc_fail(self):
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")
        t.input_scalar("a0", 1073741824)  # large size to trigger malloc failure
        t.call("initialize_zero")
        t.execute(code=26)
